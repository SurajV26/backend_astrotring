...
use App\Models\User;
use App\Helpers\AstrologyKnowledgeBuilder;
...
class AiChatApiController extends Controller
{
    ...
    {
        $chart = UserAstrologyChart::where('user_id', $user->id)->first();

        dd([
            'attributes' => $chart->getAttributes(),
            'raw_data_attr_exists' => array_key_exists('raw_data', $chart->getAttributes()),
            'raw_data_attribute' => $chart->getAttributes()['raw_data'] ?? 'NOT FOUND',
            'raw_data_property' => $chart->raw_data,
        ]);
    }
}